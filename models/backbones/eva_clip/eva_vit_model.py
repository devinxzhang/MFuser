...
class EVAVisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.adapter = nn.Sequential(*[MVFuser(self.embed_dim, d_state=16) for i in range(depth)]) 
    ...
    def extract_feats(self, x, use_fpn=True, use_adapter=True):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if use_adapter:
                dinov2_eva = self.adapter[i](torch.cat((dinov2_x, x), dim=1))
                dinov2_x = dinov2_x + dinov2_eva[:, :dinov2_x.shape[1], :]
                x = x + dinov2_eva[:, dinov2_x.shape[1]:, :]
